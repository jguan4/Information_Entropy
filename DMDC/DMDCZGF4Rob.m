function info = DMDCZGF4Rob(ims,inds,segments,micromodel,microCoords,delays,numdims,kappa,k,sigma,alpha,videoTitle,pr,k2)
% EXAMPLE CALLS
...
if (nargin<7) numdims=16; end
if (nargin<8) kappa=1/max(delays); end
if (nargin<9) k=256; end
if (nargin<10) sigma=0; end
if (nargin<11) alpha = 1/2; end
if (nargin < 12) videoTitle = 'tempVid.mpeg'; end

info.ims = ims;
info.height = size(ims,2);
info.width = size(ims,3);
if (inds == 0) inds = 1:size(ims,1); end
info.inds = inds;
