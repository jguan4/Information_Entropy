...
% old parameters to call files, do not use for future files
time_n_ahead = 2;
space_n_ahead = 2;
full = 1;
...
for m=1:10 %file index
    ...
    if full ==1
        space1 = load(strcat(savepath,'\',trial_stamp,time_stamp, '_space_h_vid_',num2str(space_n_ahead),'_',num2str(full),'_transposed.mat'));
    ...
    plot(space_yval_ave_1);
    xlim([0 sizeInd_space])
    title(strcat('Space h'))
    subplot(3,1,2)
    plot(time_h_ave_resize);
    xlim([0 sizeInd_space])
    title(strcat('Time h'))
